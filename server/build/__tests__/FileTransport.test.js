...
function makeRegistration(agentId, overrides) {
    return {
        agent_id: agentId,
...
function makeEvent(sender, recipient, body) {
...
describe("FileTransport", () => {
    ...
    describe("agent registration", () => {
        ...
        it("rejects agent IDs with path traversal characters", async () => {
            ...
            await transport.start();
            await expect(transport.registerAgent(makeRegistration("../evil@host"))).rejects.toThrow("Invalid agent ID");
        });
    ...
    describe("messaging", () => {
        ...
        it("filters messages by since_ts", async () => {
            ...
            expect(messages).toHaveLength(1);
            expect(messages[0].content.body).toBe("new");
            await sender.stop();
        });
        it("ignores .tmp- files when reading", async () => {
            ...
            fs.writeFileSync(path.join(inboxDir, ".tmp-orphan-12345"), '{"garbage": true}');
            const messages = await transport.readMessages("agent-b@host");
    ...
    describe("chokidar watcher", () => {
        it("fires onMessage callback for new inbox files", async () => {
            ...
            fs.writeFileSync(path.join(inboxDir, `${Date.now()}-${event.event_id}.json`), JSON.stringify(event));
            // Wait for chokidar to pick it up
            await new Promise((r) => setTimeout(r, 500));
            expect(received).toHaveLength(1);
            expect(received[0].content.body).toBe("realtime!");
        });
    });
});
//# sourceMappingURL=FileTransport.test.js.map