...
function makeMockTransport(): Transport & { sent: MatrixEvent[]; mockMessages: MatrixEvent[] } {
  const mock = {
    sent: [] as MatrixEvent[],
    mockMessages: [] as MatrixEvent[],
    start: async () => {},
    stop: async () => {},
    send: async (event: MatrixEvent) => { mock.sent.push(event); },
    registerAgent: async (_agent: AgentRegistration) => {},
    unregisterAgent: async (_agentId: string) => {},
    discoverAgents: async (): Promise<AgentRecord[]> => [],
    readMessages: async (_agentId: string, _filter?: MessageFilter): Promise<MatrixEvent[]> => mock.mockMessages,
...
describe("MessageStore", () => {
  ...
  it("delivers event to transport.send()", async () => {
    ...
    expect((transport.sent[0].content as { body: string }).body).toBe("delivered");
  });
